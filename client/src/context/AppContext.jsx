...
const AppContextProvider = ({ children }) => {
    ...
    const addToCart = async (product_id, quantity) => {
        ...
        if (token) {
            try {
                let response = await axios.post(`${backendUrl}/api/cart/addtocart/${userId}`, { product_id, quantity }, {
                    headers: {
                        Authorization: `${token}`
                    },
