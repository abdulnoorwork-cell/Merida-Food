        ...
        >
            ...
            <figure onClick={() => { navigate(`/shop/${product?._id}`); scrollTo(0, 0) }} className='relative overflow-hidden w-full h-full'>
                ...
                <img
                    ...
                    alt={product?.name}
                    className="w-full object-cover transform group-hover:scale-115 transition duration-500"
                />
                {/* Hover Image */}
                {product?.images?.[1]?.url && <img
                    src={product?.images?.[1]?.url}
                    alt="hover"
                    className="sm:block hidden bg-gray-100 cursor-pointer absolute top-0 left-0 w-full object-contain opacity-0 group-hover:opacity-100 transition duration-300"
                />}
            </figure>
            ...
            <div className="sm:p-7 p-5">
                ...
                <p onClick={() => { navigate(`/shop/${product?._id}`); scrollTo(0, 0) }} className="xl:text-xl text-lg font-medium leading-[1.3em] mt-2 line-clamp-1" style={{fontFamily:'Merriweather'}}>{product.name}</p>
